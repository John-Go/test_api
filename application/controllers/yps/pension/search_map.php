...
class Search_map extends CI_Controller {
	...
	function index() {
		...
		//$travelResult = $this->travel_model->travelMapLists($latitude,$longitude);
		...
		foreach ($prnsionResult['query'] as $row) {
			...
			$ret['pensions']['lists'][$pensionNo]['latitude'] = $row['mpsMapY'];	// 여행지경도
			$ret['pensions']['lists'][$pensionNo]['longitude'] = $row['mpsMapX']; // 여행지위도
			$ret['pensions']['lists'][$pensionNo]['image'] = 'http://img.yapen.co.kr/pension/basic/'.$row['mpIdx'].'/480x0/'.$row['ppbImage'];
            $ret['pensions']['lists'][$pensionNo]['likeCount'] = $this->pension_model->pensionBasketCount($row['mpIdx']);
		...
		foreach ($travelResult['query'] as $row) {
			...
			$ret['travels']['lists'][$travelNo]['latitude'] = $row['dniGoogleY'];	// 여행지경도
			$ret['travels']['lists'][$travelNo]['longitude'] = $row['dniGoogleX']; // 여행지위도
			/* 
			$ret['travels']['lists'][$travelNo]['idx'] = $row['mpIdx'];
			$ret['travels']['lists'][$travelNo]['name'] = $row['mpsName']; // 여행지명
			$ret['travels']['lists'][$travelNo]['address'] = $row['mpsAddr1']; // 여행지주소
			$ret['travels']['lists'][$travelNo]['latitude'] = $row['mpsMapX']; // 여행지위도
			$ret['travels']['lists'][$travelNo]['longitude'] = $row['mpsMapY'];	// 여행지경도
			*/
			$travelNo++;
		}
