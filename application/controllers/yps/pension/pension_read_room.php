...
class Pension_read_room extends CI_Controller {
	...
	function index() {
        ...
        foreach ($result as $row) {
            ...
            $ret['lists'][$no]["name"] = rawurlencode($row['pprName']); // 객실명
            ...
            if(	(date('Y-m-d') >= YAPEN_SALE_EVENT_START && date('Y-m-d') <= YAPEN_SALE_EVENT_END && $row['ppbReserve'] == "R") ||
                ...
                $ret['lists'][$no]["price"] = number_format(floor(($row['price']-($row['price']*0.02))/10)*10)."";  // 이용요금
            }else{
