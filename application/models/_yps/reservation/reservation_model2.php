<?php
class Reservation_model2 extends CI_Model {
    ...
    private function findPensionRoomPriceQueryOrderBy($date, $num, $priceMin, $priceMax, $arrayHolidayDate, $cnt, $tot, $arrRoomKey){
        ...
        $schQuery .= "+(SELECT MIN(ppdpSaleDay".$numOfWeek.") AS price FROM pensionPrice AS PPDP WHERE ppdpSaleDay".$numOfWeek." > 0 AND PPDP.mpIdx = PS.mpIdx AND '".substr($date,5)."' BETWEEN PPDP.ppdpStart AND PPDP.ppdpEnd AND PPDP.pprIdx NOT IN ('".$arrRoomKey."') )";
	...
	public function findReservationPensionRoomList($arrRoomKey, $personNum, $priceMin, $priceMax, $arrayDate, $arrayHolidayDate, $mpIdx){
       ...
       if($_SERVER['REMOTE_ADDR']=='211.119.165.88'){
       	echo "<pre>-------start----------".$this->db->last_query()."-------end----------</pre> ";
       }
        return $return;
	...
	function findReservationPensionLists($blockPensionKey, $holidayDate, $arrayDate, $searchLoc, $personNum, $searchPriceMin, $searchPriceMax, $searchTheme){
		if($blockPensionKey != ''){
	...
	function reservationHolidayDate($arrayDate){
		foreach($arrayDate as $arrayDate){
			$this->db->where('hDate', DATE(SUBDATE($arrayDate, INTERVAL ."1". DAY)));
			$result = $this->db->get('holiday')->result_array();
			echo $this->db->last_query().'aa';
			
			$query = 'SELECT * FROM `holiday` WHERE hDate = DATE(SUBDATE('.$arrayDate.', INTERVAL 1 DAY))';
			$holida  = $this->db->query($price_sql)->row_array();
			return $result;
		}
		
	}
	
	function getPensionList($searchLoc, $personNum, $searchPriceMin, $searchPriceMax, $searchTheme,$searchOrderby, $limit){
		$this->db->select("MPS.mpsIdx, MPS.mpIdx, MPS.mpsName, PPB.ppbImage, PPB.ppbRoomMin, PPB.ppbReserve, PPB.ppbGrade");
		...
		if($searchLoc){
			$this->db->join('pensionDB.placeTheme as PT','PT.mpsIdx = MPS.mpsIdx','LEFT');
			$this->db->where('PT.mtCode',$searchLoc);
		}
		if($searchTheme != ""){
			$searchThemeCode = array();
            if(substr($searchTheme,0,1) != "1"){
		...
		$this->db->join('pensionDB.placePensionBasic as PPB','PPB.mpIdx = MPS.mpIdx','LEFT');
		
		if($searchOrderby == "" || !$searchOrderby){
            $searchOrderby = "1";
        }
        if($searchOrderby == "1"){
            $this->db->order_by('PPB.ppbGrade DESC, Rand()');
        }else if($searchOrderby == "2"){
            $this->db->order_by('PPB.ppbWantCnt DESC, Rand()');
        }else if($searchOrderby == "4"){
		...
		$result = $this->db->get('pensionDB.mergePlaceSite as MPS',$limit)->result_array();
		
		return $result;
	}

	function getRoomList($mpIdx, $revDate, $endCount,  $personNum, $searchPriceMin, $searchPriceMax, $limit){
		if($revDate){
			$this->db->where('R.rStartDate > ',$revDate[0]);
			$this->db->where('R.rEndDate < ',$revDate[$endCount]);
		}
		
		$this->db->get('placePensionRoom AS PPR')->result_array();
