...
class Reservation_model extends CI_Model {
    ...
    function getEmptyPensionLists($searchLoc, $searchDate, $searchDateNum, $schPeople, $searchPriceMin, $searchPriceMax, $searchTheme, $idxStrings, $searchOrderby, $limit, $todaySale = 'N'){
        $setDate = explode("-", $searchDate);
        $basicPriceQuery = "(0";
        $priceQuery = "(0";
        $percentQuery = "(0";
		$todaySaleSelectQuery = "(0";
        $joinQuery = "";
        $dateBt = "";
        for($i=0; $i< $searchDateNum; $i++){
			...
			if($date == date('Y-m-d')){
				$joinQuery .= " LEFT JOIN pensionTodaySale AS PTS".$i." ON PTS".$i.".mpIdx = PP".$i.".mpIdx AND PTS".$i.".pprIdx LIKE CONCAT('%',PP".$i.".pprIdx,'%') AND '".date('Y-m-d')."' BETWEEN PTS".$i.".ptsStart AND PTS".$i.".ptsEnd AND '".date('H:i')."' BETWEEN PTS".$i.".ptsStartTime AND PTS".$i.".ptsEndTime AND PTS".$i.".ptsOpen = '1' AND PTS".$i.".ptsDay".$numOfWeek." = '1'";
				$todaySaleQuery = "/100*(100-IFNULL(PTS".$i.".ptsSale,0))";
				$todaySaleSelectQuery .= "+ (PTS".$i.".ptsSale) ";
			}else{
				$todaySaleQuery = "";
				$todaySaleSelectQuery .= "";
			}
			
			
            
            $basicPriceQuery .= "+ ( CASE WHEN PE".$i.".peIdx THEN
                                        CASE PE".$i.".peDay 
                                            WHEN '1' THEN MIN(PP".$i.".ppdpDay1)
                                            WHEN '5' THEN MIN(PP".$i.".ppdpDay5)
                                            WHEN '6' THEN MIN(PP".$i.".ppdpDay6)
                                            WHEN '7' THEN MIN(PP".$i.".ppdpDay7)
                                        ELSE MIN(PP".$i.".ppdpDay".$numOfWeek.")
                                        END
                                    ELSE
                                        MIN(PP".$i.".ppdpDay".$numOfWeek.")
                                    END )";
            $priceQuery .= "+ ( CASE WHEN PE".$i.".peIdx THEN
                                    CASE PE".$i.".peDay
                                        WHEN '1' THEN MIN(PP".$i.".ppdpSaleDay1".$todaySaleQuery.")
                                        WHEN '5' THEN MIN(PP".$i.".ppdpSaleDay5".$todaySaleQuery.")
                                        WHEN '6' THEN MIN(PP".$i.".ppdpSaleDay6".$todaySaleQuery.")
                                        WHEN '7' THEN MIN(PP".$i.".ppdpSaleDay7".$todaySaleQuery.")
                                    ELSE MIN(PP".$i.".ppdpSaleDay".$numOfWeek.$todaySaleQuery.")
                                    END
                                ELSE
                                    MIN(PP".$i.".ppdpSaleDay".$numOfWeek.$todaySaleQuery.")
                                END )";
            $percentQuery .= "+ ( CASE WHEN PE".$i.".peIdx THEN
                                    CASE PE".$i.".peDay
                                        WHEN '1' THEN MIN(PP".$i.".ppdpPercent1)
                                        WHEN '5' THEN MIN(PP".$i.".ppdpPercent5)
                                        WHEN '6' THEN MIN(PP".$i.".ppdpPercent6)
                                        WHEN '7' THEN MIN(PP".$i.".ppdpPercent7)
                                    ELSE PP".$i.".ppdpPercent".$numOfWeek."
                                    END
                                ELSE PP".$i.".ppdpPercent".$numOfWeek."
                                END )";
        ...
        $orderbyQuery = "";
        if($searchOrderby == "1"){
            $orderbyQuery = " ORDER BY topIdx DESC, PB.ppbGrade DESC, RAND(), PB.ppbMainPension DESC, RAND()";
        }else if($searchOrderby == "2"){
            $orderbyQuery = " ORDER BY PB.ppbWantCnt DESC, RAND()";
        }else if($searchOrderby == "4"){
            $orderbyQuery = " ORDER BY price ASC, RAND()";
        }else if($searchOrderby == "5"){
            $orderbyQuery = " ORDER BY price DESC, RAND()";
        }else{
            $orderbyQuery = " ORDER BY topIdx DESC, PB.ppbGrade DESC, RAND(), PB.ppbMainPension DESC, RAND()";
        }
        if($dateBt != ""){
		...
		if($todaySale == "Y"){
			$todaySaleHaving = " AND todaySale > 0"; 
		}else{
			$todaySaleHaving = "";
		}
        ...
        if($searchTheme == ""){
            ...
            $themeArray = explode(",", $searchTheme);
        }
        
        $themeQuery = "";
        for($i=0; $i< count($themeArray); $i++){
            $themeQuery .= " AND PTF.PS".trim(str_replace(".","",$themeArray[$i]))." = '1'";
        }
        
        $schQuery = "   SELECT COUNT(*) AS totalCount FROM (SELECT COUNT(MPS.mpIdx) AS totalCount,
                        ".$basicPriceQuery." AS basicPrice, ".$priceQuery." AS price, ".$percentQuery." AS percent, ".$todaySaleSelectQuery." AS todaySale
                        ...
                        LEFT JOIN placePensionBlock AS PPB ON PPR.pprIdx = PPB.pprIdx AND PPB.ppbDate IN ('".$dateBt."')
                        ".$joinQuery."
                        LEFT JOIN placePensionThemeFlag AS PTF ON MPS.mpIdx = PTF.mpIdx
                        WHERE MPS.mpsOpen = '1'
                        AND MPS.mmType = 'YPS'
                        AND MPS.mpType = 'PS'
                        AND PB.ppbReserve = 'R'
                        AND PT.mtCode IN ('".$searchLoc."')
                        AND PPB.ppbIdx IS NULL
                        AND PPR.pprIdx IS NOT NULL";
        if($themeQuery != ""){
            $schQuery .= $themeQuery;
        }
        if($searchDate >= date('Y')."-07-01" && $searchDate <= date('Y')."-08-31"){
            $schQuery .= " AND PB.ppbDateCheck = '1'";
        }
        $schQuery .= "  GROUP BY MPS.mpIdx
                        HAVING ".$priceLimitQuery." ".$todaySaleHaving." ) AS total";
        $totalCount = $this->db->query($schQuery)->row_array();
        $result['count'] = $totalCount['totalCount'];
        ...
        $schQuery = "   SELECT MPS.mpsName, MPS.mpsIdx, MPS.mpsAddr1, MPS.mpsAddr2, MPS.mpsAddr1New, MPS.mpsAddrFlag, MPS.mpsMapX, MPS.mpsMapY, MPS.mpIdx,
                        ...
                        LEFT JOIN placePensionRoom AS PPR ON PPR.mpIdx = MPS.mpIdx AND PPR.pprInMax >= ".$schPeople." AND PPR.pprOpen = '1'
                        LEFT JOIN placePensionBlock AS PPB ON PPR.pprIdx = PPB.pprIdx AND PPB.ppbDate IN ('".$dateBt."')
                        LEFT JOIN appLocTopRollingBanner AS ATRB ON '".date('Y-m-d')."' BETWEEN ATRB.altrbStartDate AND ATRB.altrbEndDate AND ATRB.altrbOpen = '1' AND ATRB.altrbLocal LIKE '%".$searchLoc."%' AND ATRB.mpIdx = MPS.mpIdx
                        ".$joinQuery."
                        LEFT JOIN placePensionThemeFlag AS PTF ON MPS.mpIdx = PTF.mpIdx
        ...
        if($idxStrings != ""){
            $schQuery .= "  AND MPS.mpIdx NOT IN (".$idxStrings.") ";
        }        
        $schQuery .= "  GROUP BY MPS.mpIdx
                        HAVING ".$priceLimitQuery." ".$todaySaleHaving."
                        ".$orderbyQuery."
                        LIMIT ".$limit;
    ...
    function getEmptyRoomLists($searchDate, $searchDateNum, $schPeople, $searchPriceMin, $searchPriceMax, $mpIdx, $todaySale = 'N'){
        $setDate = explode("-", $searchDate);
        $basicPriceQuery = "(0";
        $priceQuery = "(0";
        $percentQuery = "(0";
        ...
        for($i=0; $i< $searchDateNum; $i++){
            ...
            if($date == date('Y-m-d')){
				$joinQuery .= "
							LEFT JOIN pensionTodaySale AS PTS".$i." ON PTS".$i.".mpIdx = PP".$i.".mpIdx AND PTS".$i.".pprIdx LIKE CONCAT('%',PP".$i.".pprIdx,'%') AND '".date('Y-m-d')."' BETWEEN PTS".$i.".ptsStart AND PTS".$i.".ptsEnd AND '".date('H:i')."' BETWEEN PTS".$i.".ptsStartTime AND PTS".$i.".ptsEndTime AND PTS".$i.".ptsOpen = '1' AND PTS".$i.".ptsDay".$numOfWeek." = '1'";
				$todaySaleQuery = "/100*(100-IFNULL(PTS".$i.".ptsSale,0))";
				$todaySaleSelectQuery .= "+ ( PTS".$i.".ptsSale )";
			}else{
				$todaySaleQuery = "";
            ...
            $priceQuery .= "+ ( CASE WHEN PE".$i.".peIdx THEN
                                    CASE PE".$i.".peDay
                                        ...
                                        WHEN '6' THEN MIN(PP".$i.".ppdpSaleDay6".$todaySaleQuery.")
                                        WHEN '7' THEN MIN(PP".$i.".ppdpSaleDay7".$todaySaleQuery.")
                                    ELSE PP".$i.".ppdpSaleDay".$numOfWeek.$todaySaleQuery."
                                    END
                                ELSE PP".$i.".ppdpSaleDay".$numOfWeek.$todaySaleQuery."
                                END )";
            $percentQuery .= "+ ( CASE WHEN PE".$i.".peIdx THEN
        ...
        if($searchPriceMax == ""){
            ...
            $priceLimitQuery = " price BETWEEN ".$searchPriceMin." AND ".$searchPriceMax;
        }
        
        if($todaySale == "Y"){
        	$todaySaleHaving = " AND ptsSale > 0";
        }else{
        	$todaySaleHaving = "";
        }
        
        $schQuery = "   SELECT PPR.*, ".$basicPriceQuery." AS basicPrice, ".$priceQuery." AS price, ".$percentQuery." AS percent, ".$todaySaleSelectQuery." AS ptsSale
                        FROM placePensionRoom AS PPR
                        LEFT JOIN placePensionBlock AS PPB ON PPB.pprIdx = PPR.pprIdx AND PPB.ppbDate IN ('".$dateBt."')
                        ".$joinQuery."
                        WHERE PPR.pprOpen = '1'
                        AND PPR.pprInMax >= '".$schPeople."'
                        AND PPR.mpIdx = '".$mpIdx."'
                        AND PPB.ppbIdx IS NULL
                        GROUP BY PPR.pprIdx
                        HAVING ".$priceLimitQuery." ".$todaySaleHaving."
    ...
    function uptReserve($rIdx, $cancelPrice){
        $this->db->where('rIdx', $rIdx);
        $this->db->set('rPriceCancel', $cancelPrice);
        $this->db->update('pensionDB.reservation');
        
        $this->db->where('rIdx', $rIdx);
        $this->db->set('rcYaCancelPrice', ($cancelPrice*15/100));
        $this->db->set('rcCancelPrice', ($cancelPrice*85/100));
        $this->db->update('pensionDB.reservationConnect');
    }
    ...
    function getRevLists($rIdx){
        $this->db->where('rIdx', $rIdx);
        $this->db->order_by('rRevDate', 'ASC');
